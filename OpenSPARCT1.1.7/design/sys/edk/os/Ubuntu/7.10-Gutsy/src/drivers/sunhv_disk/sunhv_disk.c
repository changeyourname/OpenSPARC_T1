...
static int sunhv_disk_make_request(request_queue_t *queue, struct bio *bio);
...
static int sunhv_disk_make_request(request_queue_t *queue, struct bio *bio)
{
    ...
    bio_endio(bio, total_len, result);
...
fail:
    result = unregister_blkdev(sunhv_disk_major, "sunhv_disk");
    if (result < 0) {
        printk(KERN_ERR "%s: unregister_blkdev returned error %d \n", DRV_NAME, result);
    }
...
static void sunhv_disk_exit(void)
{
    ...
    error = unregister_blkdev(sunhv_disk_major, sunhv_disk_name);
    if (error < 0) {
        printk(KERN_ERR "%s: unregister_blkdev failed with error %d \n", DRV_NAME, error);
    }
