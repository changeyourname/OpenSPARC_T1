...
static irqreturn_t snet_interrupt(int irq, void *dev_id)
{
	...
	netif_rx(skb);
	pkt_count++;
	snetp->stats.rx_bytes += skb->len;
	snetp->stats.rx_packets++;
    }
...
static int sunhv_net_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	error = -EIO;
        ...
        error = 0;
	snetp->stats.tx_packets++;
	snetp->stats.tx_bytes += pkt_len;
    }
...
static void sunhv_net_init(struct net_device *dev)
{
    ...
    dev->open = sunhv_net_open;
    dev->stop = sunhv_net_stop;

    dev->set_multicast_list = NULL;
    dev->set_mac_address = sunhv_net_set_mac_address;
    dev->tx_timeout = NULL;

    dev->get_stats = sunhv_net_stats;
    dev->hard_start_xmit = sunhv_net_tx;
...
static int __devinit sunhv_net_probe(struct of_device *of_dev, const struct of_device_id *match)
{
    ...
    request_irq(of_dev->irqs[0], snet_interrupt, 0, dev->name, snetp);

    printk(KERN_INFO "%s: sunhv_net_probe succeeded \n", dev->name);
    return 0;
}

static int __devexit sunhv_net_remove(struct of_device *of_dev)
{
...
static struct of_platform_driver sunhv_net_driver = {
	.name		= "sunhv_net",
	.match_table	= sunhv_net_match,
	.probe		= sunhv_net_probe,
	.remove		= __devexit_p(sunhv_net_remove),
};
...
static int __init sunhv_net_module_init(void)
{
    ...
    err = of_register_driver(&sunhv_net_driver, &of_bus_type);
    if (err != 0) {
...
{
    of_unregister_driver(&sunhv_net_driver);
