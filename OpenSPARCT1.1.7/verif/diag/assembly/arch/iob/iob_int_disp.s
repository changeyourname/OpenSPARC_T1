...
* 
* OpenSPARC T1 Processor File: iob_int_disp.s
...
th_main_0:    
        ...
        set     0x0,            %g3     ! Current thread to receive requests.
        set     0x0,            %g7     ! Thread to generate requests.

        ! Loop through all threads.
thread_loop:
        cmp     %g7,    %g3     ! Do not generate request to itself.
        beq     skip_req
        nop
...
clr_sema_rst:           
        ...
        ! reset request and it needs to be set t0 0x1.
issue_rst:		
        set     0x1,    %l1
        set     0x0,    %l2
        sll     %g3,    0x8,    %l2
...
wait_rst_req_loop:        
        ...
        ! Read thread status for sparc0 only.
