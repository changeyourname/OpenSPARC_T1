...
* 
* OpenSPARC T1 Processor File: pio_dma_basic_cross.s
...
***                     doing memory access to the locations which DMA is done
***
**********************************************************************/
#define JBI_CONFIG
...
! add the PIO loads and stores here.
  ...
  add   %g0, 0x8, %l6           ! loop count 4, all ch

loop:
        setx    0x0000001170000000,     %l0,    %l3
        ldx     [%l3],    %l5
        add     %l3, 0x8, %l3
        deccc   %l6
        bne     loop
