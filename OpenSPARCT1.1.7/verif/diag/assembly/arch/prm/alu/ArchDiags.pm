...
sub genRandomInst
{
  ...
  while($self -> {opt} -> {random_inst} > 0 )
  {
    ...
    if($self -> {inst} -> {i} -> {value})
    {
      ...
      printf ("        setx     0x%08x%08x,   %%r%-2d,   %%r%-2s\n", $rs1_even,$rs1_odd,$tempReg, $self -> {inst} -> {rs1} -> {value});
    }  
    else
    {
      $tempReg = $self -> selTempReg();
      ...
      { 
        ...
        printf ("        setx     0x%08x%08x,   %%r%-2d,   %%r%-2s\n", $rs2_even,$rs2_odd,$tempReg, $self -> {inst} -> {rs2} -> {value});
      }
    }
    $self-> printInst();
...
sub printInst
{
   ...
   if($self-> {opt} -> {rsvd})
   {
     ...
     if ($self-> {inst} -> {i}  -> {value})
     {
       $inst .=  sprintf "0x%04x,  ",   $self-> {inst} -> {imm13}  -> {value};
     }
     else
     {
...
my ($rs1_odd, $rs1_even, $rs2_odd, $rs2_even, $imm13, $tempReg);
  ...
  if($self -> {opt} -> {negative})
  {
   ...
   #### rs1 -ve rs2 -ve  |rs1| > |rs2|
    ...
    else
    {
      ...
      printf ("        setx     0x%08x%08x,   %%r%-2d,   %%r%-2s\n", $rs1_even,$rs1_odd,$tempReg, $self -> {inst} -> {rs2} -> {value});
    }
    $self-> printInst();
  ...
  if($self -> {opt} -> {overflow})
  {
    ...
    {
      $rs2_even =  0x00000000;
      $rs2_odd  =  (int rand(0x7fffffff) ^ 0xffffffff ) + 1;
      printf ("        setx     0x%08x%08x,   %%r%-2d,   %%r%-2s\n", $rs2_even,$rs2_odd,$tempReg, $self -> {inst} -> {rs2} -> {value});
    }
    $self-> printInst();
