...
* 
* OpenSPARC T1 Processor File: err_l2cache_data_atom_cecc.s
...
#define  L2_ENTRY_PA      0x311594500
...
enable_l1:
  ...
  setx  TEST_DATA, %l0, %l2
  setx  0xdeadbeef, %l0, %l1
  mov   %l1, %l0                     ! Keep a copy

error_address:
  casxa [%g1] ASI_PRIMARY, %l2, %l1  ! Contents in %l1 & [%g1] should swap
  ...
  ! L2 should not have error now -- meaning MEC should not be set in L2 ESR
  ldx   [%g1], %l3
  cmp   %l0, %l3
  bne   %xcc, test_fail
  nop
  ...
  or    %l0, %l3, %l0            ! OR in Syndrome for [127:96]
