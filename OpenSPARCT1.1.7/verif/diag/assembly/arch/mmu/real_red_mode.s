...
* 
* OpenSPARC T1 Processor File: real_red_mode.s
...
!!		3. Set the RED mode bit and come back to PRIV mode
!!	Thr2: HYPervisor
!!		1. Use VA as the phy addr directly
!!		2. Set the RED mode bit and stay in HYP mode
!!
...
#define RED_Mode_Other_Reset done;nop
...
red_mode_loop:
	...
	nop
        ...
        add  %i1, 256, %i1
        sub     %i2, 0x1, %i2
        brnz    %i2, red_mode_loop
        nop

get_out_of_red:
! Otherwise good trap will not work 

	ta      T_CHANGE_HPRIV
        nop

	rdhpr 	%hpstate, %i7
	and	%i7,0xffffffffffffffdf, %i7
	wrhpr	%i7, %g0, %hpstate
...
! Access Va directly
	...
	setx THR0_BASE_DATA_ADDR, %l1, %l2
