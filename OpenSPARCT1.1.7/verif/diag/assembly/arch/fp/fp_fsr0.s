...
* 
* OpenSPARC T1 Processor File: fp_fsr0.s
...
/*******************************************************
 Create unused mask for fsr
 *******************************************************/

	! [64:38] reserved 
	! [29:28] reserved
	! [22] NS - should always be 0 #437 (??)
	! [19:17] ver - should be 0
	! [16:14] ftt - should be 0
	! [13] qne - should be 0 Bug #437 (??)
	! [12] reserved
	!	 [63:40]  [39:36][35:32][31:28][27:24][23:20][19:16][15:12][11:0]
	! Mask : 0xffffff_c_______0______3______0______0______f______d______000

	setx		int_data_x, %l0, %l1
	setx		fsr_reserved_mask, %l0, %l2
	setx		scratch, %l0, %l3

	set		0x0, %o7			! Error count

	ldx		[%l1+0x18], %fsr		! all 1s

	stx		%fsr, [%l3+0x0]			! temporary store

	ldx		[%l2+0x0], %g1			! load the mask
	ldx		[%l3+0x0], %g2			! Load fsr - all resrved should be 0
...
tgt0:
	...
	! Check for accural of exceptions

	setx		fsr_tem_dis_dp, %l0, %l3	! tem disabled
	ldx		[%l3+0x0], %fsr

	setx		sp_n_u_signaling_nan, %l0, %l3
	ld		[%l3+0x0], %f0
	setx	sp_p_0, %l0, %l3
  ld		[%l3+0x0], %f1
	fdivs		%f0, %f1, %f1			! This should cexc.nv

	setx		sp_p_u_normal, %l0, %l3
	ld		[%l3+0x0], %f0

	setx		sp_n_l_normal, %l0, %l3
	ld		[%l3+0x0], %f1

	fdivs		%f0, %f1, %f2			! Infinitely large number - of
	fdivs		%f1, %f0, %f3			! Infinitely small number - uf
	...
	! Put the above in a loop with tem enabled

	setx		fsr_tem_en_each_dp, %l0, %l5	! tem disabled

	set		0x14, %g1		! Max loop count
	set		0x0, %g2		! loop iterator	

	fsr_tem_rd_loop:			! Rounding mode loop
	...
	bne,a		fsr_tem_rd_loop		! If not 0, go to test more
        nop
...
fsr_reserved_mask:
	!	 [63:40]  [39:36][35:32][31:28][27:24][23:20][19:16][15:12][11:0]
	! Mask : 0xffffff_c_______0______3______0______4______f______f______000
	! Mask : 0xffffff_c_______0______3______0______0______f______d______000

	.xword		0xffffffc0300fd000

fsr_tem_dis_dp:
	.xword		0x0000000000000000	! TEM - all disabled

fsr_tem_en_each_dp:
	.xword		0x0000000008000000	! TEM.nv - rd=0
	.xword		0x0000000004000000	! TEM.of - rd=0
	.xword		0x0000000002000000	! TEM.uf - rd=0
	.xword		0x0000000001000000	! TEM.dz - rd=0
	.xword		0x0000000000800000	! TEM.nx - rd=0
	.xword		0x0000000048000000	! TEM.nv - rd=1
	.xword		0x0000000044000000	! TEM.of - rd=1
	.xword		0x0000000042000000	! TEM.uf - rd=1
	.xword		0x0000000041000000	! TEM.dz - rd=1
	.xword		0x0000000040800000	! TEM.nx - rd=1
	.xword		0x0000000088000000	! TEM.nv - rd=2
	.xword		0x0000000084000000	! TEM.of - rd=2
	.xword		0x0000000082000000	! TEM.uf - rd=2
	.xword		0x0000000081000000	! TEM.dz - rd=2
	.xword		0x0000000080800000	! TEM.nx - rd=2
	.xword		0x00000000c8000000	! TEM.nv - rd=3
	.xword		0x00000000c4000000	! TEM.of - rd=3
	.xword		0x00000000c2000000	! TEM.uf - rd=3
	.xword		0x00000000c1000000	! TEM.dz - rd=3
	.xword		0x00000000c0800000	! TEM.nx - rd=3
.align 128
...
T0_Fp_exception_ieee_754:	
	...
	rdpr	%tstate, %i1
	rdpr	%tt, %i1
	setx	trap_scratch, %l0, %l6
	stx	%fsr, [%l6+0x0]
	ldx	[%l6+0x0], %fsr		! Need to test the sync operation
	done
	nop
