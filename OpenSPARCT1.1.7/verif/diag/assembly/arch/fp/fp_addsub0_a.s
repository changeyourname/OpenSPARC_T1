...
* 
* OpenSPARC T1 Processor File: fp_addsub0_a.s
 ...
 *
 *  Description:
 *  	This test is intended test fp add and sub
 *
 **********************************************************************/
...
/*  Basic fp add and sub */
	...
	faddq		%f0, %f0, %f0		! This should cause - unimplemented fpop
	fsubq		%f0, %f0, %f0
	...
	ld		[%l4+0x0], %f0		! 
	ld		[%l6+0x0], %f4		! 
	fsubs		%f0, %f4, %f6		! Should result in negative overflow
	...
	ld		[%l4+0x0], %f0		! 
	ld		[%l6+0x0], %f4		! 
	fsubs		%f4, %f0, %f8		! Should result in negative underflow
	...
	faddsub_rd_loop0:			! Rounding mode loop
	...
	bne,a		faddsub_loop0		! If not 0, go to test more
        nop

	add		%g7, 0x1, %g7
	subcc		%g7, %g6, %g0

	bne,a		faddsub_rd_loop0	! If not 0, go to test more
        nop
