...
* 
* OpenSPARC T1 Processor File: tlu_stb2b_trap_100.s
...
*/
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!! Diag: tlu_stb2b_trap_100.s
!! No. Threads: 1
!! Description: 
!! This diag tests the stb2b lsu and fpu asynchronous traps
!! Uses user evenet based DCACHE error injection
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! Enable Error Traps
	...
	add	%l2, %l2, %l2
st_dc_tag_err:
!$EV error(0,expr(@VA(.MAIN.st_dc_tag_err), 16, 16),1,DC_TAG,1)
	sth	%l2, [%l0]
fp_div_by_zero:
	fdivs   %f8, %f0, %f4
	nop
...
wait_1:	cmp	%g1, %g0
	...
	nop
ld_dc_tag_err:
!$EV error(0,expr(@VA(.MAIN.ld_dc_tag_err), 16, 16),1,DC_TAG,1)
	ldsh	[%l0], %l4
	nop
	nop
	nop
