...
* 
* OpenSPARC T1 Processor File: dtlb_err_frf_err.s
...
*/
!DTLB parity error on the seventh store followed
!by a fp store with correctable ecc error
	
#define CIOP
#define "enable_traps.h"		
...
#define SUN_HT0_Corrected_ECC_error_0x63
	retry; \
	nop ;\
	nop
...
!$EV error(0,expr(@VA(.MAIN.dtlb_err), 16, 16),2,DTLB_DATA,35); 
	stx  %r5, [%r16]
frf_ce:
!$EV error(0,expr(@VA(.MAIN.frf_ce), 16, 16),2,FRF,x,x,x, x,x,x, ce,66,x)
	std  %f0, [%r16]
	stx  %r5, [%r16]
	stx  %r5, [%r16]		
	ldx  [%r16], %r6
