...
* 
* OpenSPARC T1 Processor File: tso_evict_fanout_ic1_8c.s
...
*/
/***********************************************************************
 * Name:   tso_evict_fanout_ic1_8c.s
 *
 * Description: I$ invalidations caused by an evict where many
...
go:
	setx    protected_area2,%l0, data_base_reg2	! the data area2
        ...
        set     barrier_code,tmp1			! meet all threads
        jmpl    tmp1,%o7				! at the barrier
        nop 

	sub	my_id_reg, 0x100, tmp1			! if I am thread 0
	brz	tmp1,  evict
	nop
goon:							! do it again or end
	dec	counter_reg
...
evict:
	ba	goon
	ld 	[data_base_reg2], test_reg		! read from data area2

!=======================================================

barrier_code:
	setx	barrier_data, tmp1, tmp2
bloop1:							! lock the barrier 
	mov	my_id_reg, tmp1				! counter
	cas	[tmp2], %g0, tmp1
	brnz	tmp1, bloop1
#ifdef PREFETCH
...
bloop3:
        ld      [tmp2 + 4], tmp3
	brz	tmp3, bout2				! if 0 we are out.
        sub     tmp3, THREAD_COUNT, tmp3                ! subtract THREAD_COUNT
        brnz    tmp3, bloop3                            ! if 0 we are also out. 
#ifdef PREFETCH
        prefetch [tmp2], #n_reads
#else
        nop
...
bout2:
        st      %g0, [tmp2 + 4]                         ! clear barrier counter
        jmpl    %o7+8, %g0                              ! return
        nop
