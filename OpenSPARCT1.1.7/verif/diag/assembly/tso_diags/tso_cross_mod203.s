...
* 
* OpenSPARC T1 Processor File: tso_cross_mod203.s
...
/***************************************************************************
***
***  Program File: tso_cross_mod203.s
***
***  Test Description : One thread modifies code, other thread(s) use it.
***                     use cas instruction to modify. Test I$/D$ exclusivity
**********************************************************************/
...
!--------------------------------------------------------------------
	...
	ld	[%l3], %o6		! load the old instruction in D$
	ld	[%l1], %o6		! load the new instructions
	ld	[%l1+4], %o7		! in o6 and o7
        mov     %o6, %i2		
	add	%l3, 8, %l4
        ld      [%l3 + 8],%i4		! use a cas and a store to modify
        cas     [%l4], %i4, %i2         ! two instructions
	st	%o7, [%l3 + 4]		! the new  instruction is a jump
!--------------------------------------------------------------------
...
wait_loop1:				! wait 
	...
	nop
        ...
        nop
goon1:
	subcc	%i1, 1, %i1		! was the new instruction the right one?
	bne	bad_end
	nop
	set	0x0,   %i1
	set	goon2, %i0
