...
## 	self.cmdRE[cmdRE] = self.handleReset
	...
	if key:
	    ...
	    # blaze command
	    if self.cmdMap:
		for keywd in self.cmdMap.cmdMap:
		    print '%s: %s' % ('CMD2', keywd)

	    found = 1

	if found == 0:
	    found = showApiDoc(key)
	if found == 0 and self.cmdMap:
	    if key in self.cmdMap.cmdMap:
		found = 1
		import sam
		sam.ui_exec('help %s' % key)
