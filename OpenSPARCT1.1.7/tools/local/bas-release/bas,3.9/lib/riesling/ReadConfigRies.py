...
# 
# OpenSPARC T1 Processor File: ReadConfigRies.py
...
class ReadConfigRies:
	...
	for i in range(MAX_CORE):
		...
		self.data[IGNORE_SPARC].append(i)
		self.data[AT_CONF].append('@conf.swvmem0.ignore_sparc =+ %d' % (i))

	# produce THREADS
	self.data[AT_CONF].append('@conf.swvmem0.threads = %s' % (tokens[1].replace('x', '0')))
